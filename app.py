...
def clean_num(s):
    ...
    s=re.sub(r"[()$]","",s)
    # Handle European period-as-thousands (X.YYY => XYYY)
...
INCOME_SKIP=re.compile(
    r"^(interest|fee|commission|trading|net\s+interest|gross|profit|tax|impairment|reversal|gains?|operating\s+income\s+before|other\s+operating)",
    re.IGNORECASE)

DED_SKIP=re.compile(r"^\s*(less|net\s+of|after\s+deducting)[:.]?\s*$", re.IGNORECASE)
...
def parse_bs(lines, section):
    ...
    for line in lines:
        ...
        # Skip wrapped-line tails (starts lowercase or with closing paren)
    ...
    def fnd(pat):
        for line in lines:
            ...
            if re.search(pat, s, re.IGNORECASE):
                # Strip inline footnote reference numbers (e.g. "Total income 2 22,513 17,957")
...
def generate_report_html(d, filename, ul, mult):
    ...
    a_narr=(f"The top 3 biggest assets {'remain the same' if set(x['label'] for x in ai_s[:3])==set(x['label'] for x in sorted([x for x in ai if x.get('prior')],key=lambda x:x['prior'],reverse=True)[:3]) else 'differ'} between the two periods. "
    ...
    l_narr=(f"The top 3 biggest liabilities {'remain the same' if set(x['label'] for x in li_s[:3])==set(x['label'] for x in sorted([x for x in li if x.get('prior')],key=lambda x:x['prior'],reverse=True)[:3]) else 'changed'} from the prior period. "
    ...
    def full_table(items,total,title):
        ...
        return f"""<h2>{title}</h2><table><thead><tr><th style='text-align:left'>Item</th>
          <th>Current ({ul})</th><th>%</th><th>Prior ({ul})</th></tr></thead><tbody>{rows}</tbody></table>"""
...
<h2>Asset Concentration &mdash; Current</h2>
{bullets(ai,tc_c,tc_p,False,False)}
<h2>Asset Concentration &mdash; Prior</h2>
{bullets(ai,tc_c,tc_p,True,False)}
<p class="narrative">{a_narr}</p>
<h2>Liability Concentration &mdash; Current</h2>
{bullets(li,tl_c,tl_p,False,True)}
<h2>Liability Concentration &mdash; Prior</h2>
